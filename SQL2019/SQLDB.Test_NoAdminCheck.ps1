...
.NOTES
    ...
    Creation Date:  29th Jan 2020
    Purpose/Change: Initial Draft
Purpose/Change: Change Mount Point to M:
...
#$TempDBVolumeSize = ([math]::Round($TempDbVolume.size / 15966540922) * 15)
#$TempDBFileSize = [Math]::Round(($TempDBVolumeSize * 0.8) / ($LogicalCpuCount + 1)) * 1536
...
Describe -Tags "SQLDB" "SQL Server Deployment" {
    ...
    It "DB Services Group is a member of local administrators" {
        (Get-LocalGroupMember -Group "Administrators" -Member "BOE\ISTD-S DB Services (SQL)").Count | Should Be 0
    }
