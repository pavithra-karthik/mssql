...
try {
	...
	try {
        ...
        $ISOVolume = Mount-DiskImage -ImagePath ($MediaPath + "SQL2019\" + $ConfigData.SqlIso) -PassThru | Get-Volume
    	$DriveInfo = Get-PSDrive -Name $ISOVolume.DriveLetter

    	TraceLog ("INFO: Copying media files from ISO to $MediaPath\SQL2019")
        robocopy /xc /xn /xo /e /np /nfl /ndl $driveInfo.Root "$MediaPath\SQL2019" | Out-Null
    	Dismount-DiskImage -ImagePath ($MediaPath + "SQL2019\" + $ConfigData.SqlIso)

    	TraceLog "INFO: Slipstreaming service pack and cumulative update"
    	New-Item ($MediaPath + "SQL2019\updates\") -ItemType "directory" -Force | Out-Null
        Copy-Item ($MediaPath + "Updates\" + $ConfigData.SqlCurrentSp) -Destination ($MediaPath + "SQL2019\updates\") -Force | Out-Null
    	Copy-Item ($MediaPath + "Updates\" + $ConfigData.SqlCurrentCu) -Destination ($MediaPath + "SQL2019\updates\") -Force | Out-Null

        TraceLog "INFO: Installing SQL Server Database Services"
        $SetupPath="$MediaPath\SQL2019\setup.exe"
